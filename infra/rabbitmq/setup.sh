...
declare PUT "queues/%2F/notification.sms" \
  '{"durable":true,"arguments":{"x-dead-letter-exchange":"notifications.dlx"}}'
declare PUT "queues/%2F/notification.in_app" \
  '{"durable":true,"arguments":{"x-dead-letter-exchange":"notifications.dlx"}}'

# Bindings: all order.* and delivery.* events fan out to all channels
for queue in notification.email notification.push notification.sms notification.in_app; do
  for routing_key in "order.*" "delivery.*" "payment.*" "agent.*"; do
